...
describe("Teste de associacao pedido e cliente", () => {
  let pedidoId: number;
  let clienteId: number;
  ...
  it("Deve criar um pedido e associar ao cliente", async () => {
    ...
    if(pedido) {
      expect(pedido.Cliente).not.toBeNull();
      if(pedido.Cliente) {
        expect(pedido.Cliente.id).toBe(clienteId);
      } 
    } 
  });
